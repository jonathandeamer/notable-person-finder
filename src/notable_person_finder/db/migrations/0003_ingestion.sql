...
-- One row per settled feed work item whose accepted handler result reaches
-- persistence: a successful/not-modified result, a not-inspected result, or
-- the final failure after retry adjudication. Deliberate no-call settlements
-- (such as a paused provider or a preparation failure) and refused
-- non-settling handler outcomes create no `feed_fetch` row. The `attempt`
-- table remains the one-row-per-external-call record. Conditional-
...
-- repository from the latest fetch for the exact `requested_url` whose
-- `outcome` is not 'failed' *and* which actually carries a validator, so a URL
-- move, a failed fetch, or a bare 304 (which usually omits Last-Modified and
-- often ETag) cannot poison the validators an earlier successful fetch for
-- that representation already established.
